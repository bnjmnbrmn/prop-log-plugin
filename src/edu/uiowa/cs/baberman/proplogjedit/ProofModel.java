...
import edu.uiowa.cs.baberman.proplogjedit.nodes.*;
...
public final class ProofModel {
	...
	private void initializeNavManipKCMTree() {
		...
		justificationKey = setToMenu.putNewSubmenu(ThirtyKey.KeyPosition.T.getVK_CODE())
				.setMenuItemText("Justification");
		
		ThirtyKey justificationMenu;
		justificationMenu = justificationKey.getSubmenu();
...
//   | 'or_intro_r ' LINE_ID
		...
		justificationMenu.putNewLeaf(ThirtyKey.KeyPosition.Q)
				.addPressAction(new AbstractAction() {

			@Override
			public void actionPerformed(ActionEvent ae) {
				if (getSelectedNode() instanceof Justification) {
					Justification selectedJustification = (Justification) getSelectedNode();
					selectedJustification.setToAndIntro();
				}
			}
		}).setMenuItemText("and_intro");
		
		justificationMenu.putNewLeaf(ThirtyKey.KeyPosition.E)
				.addPressAction(new AbstractAction() {
			...
			public void actionPerformed(ActionEvent ae) {
				if (getSelectedNode() instanceof Justification) {
					Justification selectedJustification = (Justification) getSelectedNode();
					selectedJustification.setToAndElimR();
				}
			}
		}).setMenuItemText("and_elim_r");
		
		justificationMenu.putNewLeaf(ThirtyKey.KeyPosition.W)
				.addPressAction(new AbstractAction() {
			...
			public void actionPerformed(ActionEvent ae) {
				if (getSelectedNode() instanceof Justification) {
					Justification selectedJustification = (Justification) getSelectedNode();
					selectedJustification.setToAndElimL();
				}
			}
		}).setMenuItemText("and_elim_l");

		justificationMenu.putNewLeaf(ThirtyKey.KeyPosition.D)
				.addPressAction(new AbstractAction() {
			...
			public void actionPerformed(ActionEvent ae) {
				if (getSelectedNode() instanceof Justification) {
					Justification selectedJustification = (Justification) getSelectedNode();
					selectedJustification.setToOrIntroR();
				}
			}
		}).setMenuItemText("or_elim_r");
		
		justificationMenu.putNewLeaf(ThirtyKey.KeyPosition.S)
				.addPressAction(new AbstractAction() {
			...
			public void actionPerformed(ActionEvent ae) {
				if (getSelectedNode() instanceof Justification) {
					Justification selectedJustification = (Justification) getSelectedNode();
					selectedJustification.setToOrIntroL();
				}
			}
		}).setMenuItemText("or_intro_l");
		
		justificationMenu.putNewLeaf(ThirtyKey.KeyPosition.A)
				.addPressAction(new AbstractAction() {
			...
			public void actionPerformed(ActionEvent ae) {
				if (getSelectedNode() instanceof Justification) {
					Justification selectedJustification = (Justification) getSelectedNode();
					selectedJustification.setToOrElim();
				}
			}
		}).setMenuItemText("or_elim");
...
//   | 'not_not_elim ' LINE_ID
		...
		justificationMenu.putNewLeaf(ThirtyKey.KeyPosition.I)
				.addPressAction(new AbstractAction() {
			...
			public void actionPerformed(ActionEvent ae) {
				if (getSelectedNode() instanceof Justification) {
					Justification selectedJustification = (Justification) getSelectedNode();
					selectedJustification.setToImplElim();
				}
			}
		}).setMenuItemText("impl_elim");
		
		justificationMenu.putNewLeaf(ThirtyKey.KeyPosition.O)
				.addPressAction(new AbstractAction() {
			...
			public void actionPerformed(ActionEvent ae) {
				if (getSelectedNode() instanceof Justification) {
					Justification selectedJustification = (Justification) getSelectedNode();
					selectedJustification.setToFalseElim();
				}
			}
		}).setMenuItemText("false_elim");
		
		justificationMenu.putNewLeaf(ThirtyKey.KeyPosition.P)
				.addPressAction(new AbstractAction() {
			...
			public void actionPerformed(ActionEvent ae) {
				if (getSelectedNode() instanceof Justification) {
					Justification selectedJustification = (Justification) getSelectedNode();
					selectedJustification.setToNotNotElim();
				}
			}
		}).setMenuItemText("not_not_elim");
		
		
		SubmenuKey<ThirtyKey> operatorKey;
		operatorKey = setToMenu.putNewSubmenu(ThirtyKey.KeyPosition.F.getVK_CODE())
	...
	private void initializeIdentifierEditingKCMTreeRootCard(ThirtyKey root) {
		...
		SubmenuKey<ThirtyKey> digitsKey = root.putNewSubmenu(ThirtyKey.KeyPosition.SEMICOLON.getVK_CODE())
	...
	private void initializePropVarDeclareKCMTree() {
		...
		propVarDeclareKCMRoot.putNewLeaf(ThirtyKey.KeyPosition.COMMA.getVK_CODE())
			...
			public void actionPerformed(ActionEvent e) {
				...
				SpacePropVar parent = (SpacePropVar) selectedPropVar.getParent();
				OneOrMoreSpacePropVars grandparent = (OneOrMoreSpacePropVars) parent.getParent();
...
//		});
	...
	public void moveLeft() {
		int index;
		index = getCurrentSelectableNodeListInclusive().indexOf(getSelectedNode());

		if (index > 0) {
			SelectableNode toSelect;
			toSelect = getCurrentSelectableNodeListInclusive().get(index - 1);
